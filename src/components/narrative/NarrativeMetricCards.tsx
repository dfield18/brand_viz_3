...
export function NarrativeMetricCards({
  ...
  if (hedgingRate != null) {
    ...
    cards.push({
      ...
      formula: `Model Confidence = 100 − Hedging Rate\n\nHedging Rate measures linguistic tentativeness in AI responses:\n1. Count distinct hedging cues in the subject-context text — modals (e.g., "may", "might", "could"), likelihood adverbs ("possibly", "perhaps", "presumably"), reporting hedges ("appears to", "seems to", "reportedly", "allegedly"), approximators ("approximately", "somewhat"), and uncertainty phrases ("it is unclear", "opinions vary")\n2. A response is "hedged" if it reaches ≥2 distinct cues — a single stray "may" doesn't count\n3. Hedging Rate = (hedged responses ÷ total responses) × 100\n\nCurrent: ${confidence}% confidence (${hedgingRate}% hedging rate)\n\nClassification:\n• ≥ 85%: Direct & confident\n• 65–84%: Some hedging\n• < 65%: Heavily hedged`,
